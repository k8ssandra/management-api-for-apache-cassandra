...
public class ObjectSerializer4x<T> implements ObjectSerializer<T>
{
    ...
    {
        serializers = GenericSerializer4x.simpleType(genericType) ?
                      ImmutableSortedMap.<String, FieldSerializer>of("result", new FieldSerializer(GenericSerializer4x.getType(genericType), x -> x)) :
                      ImmutableSortedMap.copyOf(Arrays.stream(clazz.getFields())
                                                  .collect(Collectors.toMap(field -> field.getName(),
                                                                            field -> new FieldSerializer(GenericSerializer4x.getType(field.getType()), field))));
