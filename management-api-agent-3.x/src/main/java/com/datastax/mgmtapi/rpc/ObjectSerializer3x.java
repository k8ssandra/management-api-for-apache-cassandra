...
public class ObjectSerializer3x<T> implements ObjectSerializer<T>
{
    ...
    {
        serializers = GenericSerializer3x.simpleType(genericType) ?
                      ImmutableSortedMap.<String, FieldSerializer>of("result", new FieldSerializer(GenericSerializer3x.getType(genericType), x -> x)) :
                      ImmutableSortedMap.copyOf(Arrays.stream(clazz.getFields())
                                                  .collect(Collectors.toMap(field -> field.getName(),
                                                                            field -> new FieldSerializer(GenericSerializer3x.getType(field.getType()), field))));
