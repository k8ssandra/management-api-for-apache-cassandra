...
public class ObjectSerializer41x<T> implements ObjectSerializer<T>
{
    ...
    {
        serializers = GenericSerializer41x.simpleType(genericType) ?
                      ImmutableSortedMap.<String, FieldSerializer>of("result", new FieldSerializer(GenericSerializer41x.getType(genericType), x -> x)) :
                      ImmutableSortedMap.copyOf(Arrays.stream(clazz.getFields())
                                                  .collect(Collectors.toMap(field -> field.getName(),
                                                                            field -> new FieldSerializer(GenericSerializer41x.getType(field.getType()), field))));
