...
public class ObjectSerializerDse68<T> implements ObjectSerializer<T>
{
    ...
    {
        serializers = GenericSerializerDse68.simpleType(genericType) ?
                      ImmutableSortedMap.<String, FieldSerializer>of("result", new FieldSerializer(GenericSerializerDse68.getType(genericType), x -> x)) :
                      ImmutableSortedMap.copyOf(Arrays.stream(clazz.getFields())
                                                  .collect(Collectors.toMap(field -> field.getName(),
                                                                            field -> new FieldSerializer(GenericSerializerDse68.getType(field.getType()), field))));
